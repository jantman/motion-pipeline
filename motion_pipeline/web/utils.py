...
def proxy_aware_redirect(path, code=302):
    if 'HTTP_X_FORWARDED_FOR' not in request.environ:
        return redirect(path, code=code)
    redir = '%s://%s:%s%s' % (
        request.environ.get('HTTP_X_FORWARDED_PROTO', 'http'),
        request.environ.get(
            'HTTP_X_FORWARDED_HOST', request.environ['HTTP_HOST']
        ),
        request.environ.get(
            'HTTP_X_FORWARDED_PORT', request.environ['SERVER_PORT']
        ),
        path
    )
    return redirect(redir, code=code)
